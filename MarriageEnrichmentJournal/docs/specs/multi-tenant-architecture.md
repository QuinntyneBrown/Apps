# Multi-Tenant Architecture Specification for MarriageEnrichmentJournal

## Overview

This document describes the multi-tenant architecture implemented in MarriageEnrichmentJournal, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

MarriageEnrichmentJournal uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`MarriageEnrichmentJournal.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`MarriageEnrichmentJournal.Infrastructure/Services/TenantContext.cs`)
...
- **JournalEntry**: `TenantId` property for tenant isolation
- **Gratitude**: `TenantId` property for tenant isolation
- **Reflection**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `MarriageEnrichmentJournalContext` DbContext applies automatic query filters:
