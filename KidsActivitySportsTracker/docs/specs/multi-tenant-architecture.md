# Multi-Tenant Architecture Specification for KidsActivitySportsTracker

## Overview

This document describes the multi-tenant architecture implemented in KidsActivitySportsTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

KidsActivitySportsTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`KidsActivitySportsTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`KidsActivitySportsTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Activity**: `TenantId` property for tenant isolation
- **Schedule**: `TenantId` property for tenant isolation
- **Carpool**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `KidsActivitySportsTrackerContext` DbContext applies automatic query filters:
