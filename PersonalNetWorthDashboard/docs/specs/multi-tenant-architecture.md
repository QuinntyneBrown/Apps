# Multi-Tenant Architecture Specification for PersonalNetWorthDashboard

## Overview

This document describes the multi-tenant architecture implemented in PersonalNetWorthDashboard, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PersonalNetWorthDashboard uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PersonalNetWorthDashboard.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PersonalNetWorthDashboard.Infrastructure/Services/TenantContext.cs`)
...
- **Asset**: `TenantId` property for tenant isolation
- **Liability**: `TenantId` property for tenant isolation
- **NetWorthSnapshot**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PersonalNetWorthDashboardContext` DbContext applies automatic query filters:
