# Multi-Tenant Architecture Specification for AnnualHealthScreeningReminder

## Overview

This document describes the multi-tenant architecture implemented in AnnualHealthScreeningReminder, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

AnnualHealthScreeningReminder uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`AnnualHealthScreeningReminder.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`AnnualHealthScreeningReminder.Infrastructure/Services/TenantContext.cs`)
...
- **Screening**: `TenantId` property for tenant isolation
- **Appointment**: `TenantId` property for tenant isolation
- **Reminder**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `AnnualHealthScreeningReminderContext` DbContext applies automatic query filters:
