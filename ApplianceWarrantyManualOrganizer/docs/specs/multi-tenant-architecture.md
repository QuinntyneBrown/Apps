# Multi-Tenant Architecture Specification for ApplianceWarrantyManualOrganizer

## Overview

This document describes the multi-tenant architecture implemented in ApplianceWarrantyManualOrganizer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

ApplianceWarrantyManualOrganizer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`ApplianceWarrantyManualOrganizer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`ApplianceWarrantyManualOrganizer.Infrastructure/Services/TenantContext.cs`)
...
- **Appliance**: `TenantId` property for tenant isolation
- **Warranty**: `TenantId` property for tenant isolation
- **Manual**: `TenantId` property for tenant isolation
- **ServiceRecord**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `ApplianceWarrantyManualOrganizerContext` DbContext applies automatic query filters:
