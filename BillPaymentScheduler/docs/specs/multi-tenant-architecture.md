# Multi-Tenant Architecture Specification for BillPaymentScheduler

## Overview

This document describes the multi-tenant architecture implemented in BillPaymentScheduler, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

BillPaymentScheduler uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`BillPaymentScheduler.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`BillPaymentScheduler.Infrastructure/Services/TenantContext.cs`)
...
- **Bill**: `TenantId` property for tenant isolation
- **Payment**: `TenantId` property for tenant isolation
- **Payee**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `BillPaymentSchedulerContext` DbContext applies automatic query filters:
