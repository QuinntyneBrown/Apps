# Multi-Tenant Architecture Specification for VideoGameCollectionManager

## Overview

This document describes the multi-tenant architecture implemented in VideoGameCollectionManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

VideoGameCollectionManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`VideoGameCollectionManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`VideoGameCollectionManager.Infrastructure/Services/TenantContext.cs`)
...
- **PlaySession**: `TenantId` property for tenant isolation
- **Wishlist**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `VideoGameCollectionManagerContext` DbContext applies automatic query filters:
