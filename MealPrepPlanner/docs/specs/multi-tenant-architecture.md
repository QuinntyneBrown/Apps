# Multi-Tenant Architecture Specification for MealPrepPlanner

## Overview

This document describes the multi-tenant architecture implemented in MealPrepPlanner, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

MealPrepPlanner uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`MealPrepPlanner.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`MealPrepPlanner.Infrastructure/Services/TenantContext.cs`)
...
- **Nutrition**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `MealPrepPlannerContext` DbContext applies automatic query filters:
