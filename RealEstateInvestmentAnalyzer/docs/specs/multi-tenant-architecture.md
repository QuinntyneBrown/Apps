# Multi-Tenant Architecture Specification for RealEstateInvestmentAnalyzer

## Overview

This document describes the multi-tenant architecture implemented in RealEstateInvestmentAnalyzer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

RealEstateInvestmentAnalyzer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`RealEstateInvestmentAnalyzer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`RealEstateInvestmentAnalyzer.Infrastructure/Services/TenantContext.cs`)
...
- **Property**: `TenantId` property for tenant isolation
- **CashFlow**: `TenantId` property for tenant isolation
- **Expense**: `TenantId` property for tenant isolation
- **Lease**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `RealEstateInvestmentAnalyzerContext` DbContext applies automatic query filters:
