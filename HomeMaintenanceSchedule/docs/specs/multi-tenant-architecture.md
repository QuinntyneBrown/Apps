# Multi-Tenant Architecture Specification for HomeMaintenanceSchedule

## Overview

This document describes the multi-tenant architecture implemented in HomeMaintenanceSchedule, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

HomeMaintenanceSchedule uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`HomeMaintenanceSchedule.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`HomeMaintenanceSchedule.Infrastructure/Services/TenantContext.cs`)
...
- **MaintenanceTask**: `TenantId` property for tenant isolation
- **ServiceLog**: `TenantId` property for tenant isolation
- **Contractor**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `HomeMaintenanceScheduleContext` DbContext applies automatic query filters:
