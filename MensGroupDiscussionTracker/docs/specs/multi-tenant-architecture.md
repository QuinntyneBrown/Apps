# Multi-Tenant Architecture Specification for MensGroupDiscussionTracker

## Overview

This document describes the multi-tenant architecture implemented in MensGroupDiscussionTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

MensGroupDiscussionTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`MensGroupDiscussionTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`MensGroupDiscussionTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Meeting**: `TenantId` property for tenant isolation
- **Topic**: `TenantId` property for tenant isolation
- **Resource**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `MensGroupDiscussionTrackerContext` DbContext applies automatic query filters:
