# Multi-Tenant Architecture Specification for ProfessionalReadingList

## Overview

This document describes the multi-tenant architecture implemented in ProfessionalReadingList, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

ProfessionalReadingList uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`ProfessionalReadingList.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`ProfessionalReadingList.Infrastructure/Services/TenantContext.cs`)
...
- **ReadingProgress**: `TenantId` property for tenant isolation
- **Note**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `ProfessionalReadingListContext` DbContext applies automatic query filters:
