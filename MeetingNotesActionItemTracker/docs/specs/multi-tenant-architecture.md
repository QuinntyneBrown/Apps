# Multi-Tenant Architecture Specification for MeetingNotesActionItemTracker

## Overview

This document describes the multi-tenant architecture implemented in MeetingNotesActionItemTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

MeetingNotesActionItemTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`MeetingNotesActionItemTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`MeetingNotesActionItemTracker.Infrastructure/Services/TenantContext.cs`)
...
- **ActionItem**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `MeetingNotesActionItemTrackerContext` DbContext applies automatic query filters:
