...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.Console()
    .WriteTo.File("logs/couplesgoaltracker-.log", rollingInterval: RollingInterval.Day)
    .CreateLogger();
...
{
    c.SwaggerDoc("v1", new() { Title = "CouplesGoalTracker API", Version = "v1" });
});
...
try
{
    Log.Information("Starting CouplesGoalTracker API");
    app.Run();
}
catch (Exception ex)
{
