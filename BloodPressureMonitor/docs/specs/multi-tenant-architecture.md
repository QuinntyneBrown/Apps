# Multi-Tenant Architecture Specification for BloodPressureMonitor

## Overview

This document describes the multi-tenant architecture implemented in BloodPressureMonitor, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

BloodPressureMonitor uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`BloodPressureMonitor.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`BloodPressureMonitor.Infrastructure/Services/TenantContext.cs`)
...
- **Reading**: `TenantId` property for tenant isolation
- **Trend**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `BloodPressureMonitorContext` DbContext applies automatic query filters:
