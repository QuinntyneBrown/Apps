# Multi-Tenant Architecture Specification for RoadsideAssistanceInfoHub

## Overview

This document describes the multi-tenant architecture implemented in RoadsideAssistanceInfoHub, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

RoadsideAssistanceInfoHub uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`RoadsideAssistanceInfoHub.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`RoadsideAssistanceInfoHub.Infrastructure/Services/TenantContext.cs`)
...
- **Vehicle**: `TenantId` property for tenant isolation
- **InsuranceInfo**: `TenantId` property for tenant isolation
- **EmergencyContact**: `TenantId` property for tenant isolation
- **Policy**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `RoadsideAssistanceInfoHubContext` DbContext applies automatic query filters:
