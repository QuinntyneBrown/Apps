# Multi-Tenant Architecture Specification for FriendGroupEventCoordinator

## Overview

This document describes the multi-tenant architecture implemented in FriendGroupEventCoordinator, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FriendGroupEventCoordinator uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FriendGroupEventCoordinator.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FriendGroupEventCoordinator.Infrastructure/Services/TenantContext.cs`)
...
- **Event**: `TenantId` property for tenant isolation
- **RSVP**: `TenantId` property for tenant isolation
- **Group**: `TenantId` property for tenant isolation
- **Member**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FriendGroupEventCoordinatorContext` DbContext applies automatic query filters:
