# Multi-Tenant Architecture Specification for JobSearchOrganizer

## Overview

This document describes the multi-tenant architecture implemented in JobSearchOrganizer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

JobSearchOrganizer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`JobSearchOrganizer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`JobSearchOrganizer.Infrastructure/Services/TenantContext.cs`)
...
- **Application**: `TenantId` property for tenant isolation
- **Company**: `TenantId` property for tenant isolation
- **Interview**: `TenantId` property for tenant isolation
- **Offer**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `JobSearchOrganizerContext` DbContext applies automatic query filters:
