# Multi-Tenant Architecture Specification for FamilyCalendarEventPlanner

## Overview

This document describes the multi-tenant architecture implemented in FamilyCalendarEventPlanner, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FamilyCalendarEventPlanner uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FamilyCalendarEventPlanner.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FamilyCalendarEventPlanner.Infrastructure/Services/TenantContext.cs`)
...
- **CalendarEvent**: `TenantId` property for tenant isolation
- **EventAttendee**: `TenantId` property for tenant isolation
- **FamilyMember**: `TenantId` property for tenant isolation
- **AvailabilityBlock**: `TenantId` property for tenant isolation
- **ScheduleConflict**: `TenantId` property for tenant isolation
- **EventReminder**: `TenantId` property for tenant isolation
- **Household**: `TenantId` property for tenant isolation
- **User**: `TenantId` property for tenant isolation
- **Role**: `TenantId` property for tenant isolation
- **UserRole**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FamilyCalendarEventPlannerContext` DbContext applies automatic query filters:
