# Multi-Tenant Architecture Specification for HomeImprovementProjectManager

## Overview

This document describes the multi-tenant architecture implemented in HomeImprovementProjectManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

HomeImprovementProjectManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`HomeImprovementProjectManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`HomeImprovementProjectManager.Infrastructure/Services/TenantContext.cs`)
...
- **Project**: `TenantId` property for tenant isolation
- **Budget**: `TenantId` property for tenant isolation
- **Contractor**: `TenantId` property for tenant isolation
- **Material**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `HomeImprovementProjectManagerContext` DbContext applies automatic query filters:
