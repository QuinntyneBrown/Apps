# Multi-Tenant Architecture Specification for PersonalProjectPipeline

## Overview

This document describes the multi-tenant architecture implemented in PersonalProjectPipeline, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PersonalProjectPipeline uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PersonalProjectPipeline.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PersonalProjectPipeline.Infrastructure/Services/TenantContext.cs`)
...
- **ProjectTask**: `TenantId` property for tenant isolation
- **Milestone**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PersonalProjectPipelineContext` DbContext applies automatic query filters:
