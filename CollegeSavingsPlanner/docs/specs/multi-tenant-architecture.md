# Multi-Tenant Architecture Specification for CollegeSavingsPlanner

## Overview

This document describes the multi-tenant architecture implemented in CollegeSavingsPlanner, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

CollegeSavingsPlanner uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`CollegeSavingsPlanner.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`CollegeSavingsPlanner.Infrastructure/Services/TenantContext.cs`)
...
- **Plan**: `TenantId` property for tenant isolation
- **Contribution**: `TenantId` property for tenant isolation
- **Beneficiary**: `TenantId` property for tenant isolation
- **Projection**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `CollegeSavingsPlannerContext` DbContext applies automatic query filters:
