# Multi-Tenant Architecture Specification for StressMoodTracker

## Overview

This document describes the multi-tenant architecture implemented in StressMoodTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

StressMoodTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`StressMoodTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`StressMoodTracker.Infrastructure/Services/TenantContext.cs`)
...
- **MoodEntry**: `TenantId` property for tenant isolation
- **Trigger**: `TenantId` property for tenant isolation
- **Journal**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `StressMoodTrackerContext` DbContext applies automatic query filters:
