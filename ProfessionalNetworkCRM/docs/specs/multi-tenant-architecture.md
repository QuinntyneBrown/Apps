# Multi-Tenant Architecture Specification for ProfessionalNetworkCRM

## Overview

This document describes the multi-tenant architecture implemented in ProfessionalNetworkCRM, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

ProfessionalNetworkCRM uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`ProfessionalNetworkCRM.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`ProfessionalNetworkCRM.Infrastructure/Services/TenantContext.cs`)
...
- **Interaction**: `TenantId` property for tenant isolation
- **FollowUp**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `ProfessionalNetworkCRMContext` DbContext applies automatic query filters:
