# Multi-Tenant Architecture Specification for RunningLogRaceTracker

## Overview

This document describes the multi-tenant architecture implemented in RunningLogRaceTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

RunningLogRaceTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`RunningLogRaceTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`RunningLogRaceTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Run**: `TenantId` property for tenant isolation
- **Race**: `TenantId` property for tenant isolation
- **TrainingPlan**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `RunningLogRaceTrackerContext` DbContext applies automatic query filters:
