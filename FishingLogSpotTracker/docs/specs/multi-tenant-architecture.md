# Multi-Tenant Architecture Specification for FishingLogSpotTracker

## Overview

This document describes the multi-tenant architecture implemented in FishingLogSpotTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FishingLogSpotTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FishingLogSpotTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FishingLogSpotTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Catch**: `TenantId` property for tenant isolation
- **Spot**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FishingLogSpotTrackerContext` DbContext applies automatic query filters:
