# Multi-Tenant Architecture Specification for MorningRoutineBuilder

## Overview

This document describes the multi-tenant architecture implemented in MorningRoutineBuilder, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

MorningRoutineBuilder uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`MorningRoutineBuilder.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`MorningRoutineBuilder.Infrastructure/Services/TenantContext.cs`)
...
- **Routine**: `TenantId` property for tenant isolation
- **RoutineTask**: `TenantId` property for tenant isolation
- **CompletionLog**: `TenantId` property for tenant isolation
- **Streak**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `MorningRoutineBuilderContext` DbContext applies automatic query filters:
