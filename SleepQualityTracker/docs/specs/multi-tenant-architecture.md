# Multi-Tenant Architecture Specification for SleepQualityTracker

## Overview

This document describes the multi-tenant architecture implemented in SleepQualityTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

SleepQualityTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`SleepQualityTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`SleepQualityTracker.Infrastructure/Services/TenantContext.cs`)
...
- **SleepSession**: `TenantId` property for tenant isolation
- **Habit**: `TenantId` property for tenant isolation
- **Pattern**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `SleepQualityTrackerContext` DbContext applies automatic query filters:
