# Multi-Tenant Architecture Specification for WarrantyReturnPeriodTracker

## Overview

This document describes the multi-tenant architecture implemented in WarrantyReturnPeriodTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

WarrantyReturnPeriodTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`WarrantyReturnPeriodTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`WarrantyReturnPeriodTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Purchase**: `TenantId` property for tenant isolation
- **Warranty**: `TenantId` property for tenant isolation
- **ReturnWindow**: `TenantId` property for tenant isolation
- **Receipt**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `WarrantyReturnPeriodTrackerContext` DbContext applies automatic query filters:
