# Multi-Tenant Architecture Specification for InvestmentPortfolioTracker

## Overview

This document describes the multi-tenant architecture implemented in InvestmentPortfolioTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

InvestmentPortfolioTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`InvestmentPortfolioTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`InvestmentPortfolioTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Account**: `TenantId` property for tenant isolation
- **Holding**: `TenantId` property for tenant isolation
- **Transaction**: `TenantId` property for tenant isolation
- **Dividend**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `InvestmentPortfolioTrackerContext` DbContext applies automatic query filters:
