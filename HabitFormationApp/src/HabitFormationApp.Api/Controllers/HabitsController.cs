...
public class HabitsController : ControllerBase
{
    ...
    [HttpPut("{habitId:guid}/toggle-active")]
    [ProducesResponseType(typeof(HabitDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<HabitDto>> ToggleActive(Guid habitId)
    {
        _logger.LogInformation("Toggling active status for habit {HabitId}", habitId);

        var result = await _mediator.Send(new ToggleHabitActiveCommand { HabitId = habitId });
