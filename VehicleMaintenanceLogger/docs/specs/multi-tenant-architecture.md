# Multi-Tenant Architecture Specification for VehicleMaintenanceLogger

## Overview

This document describes the multi-tenant architecture implemented in VehicleMaintenanceLogger, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

VehicleMaintenanceLogger uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`VehicleMaintenanceLogger.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`VehicleMaintenanceLogger.Infrastructure/Services/TenantContext.cs`)
...
- **MaintenanceSchedule**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `VehicleMaintenanceLoggerContext` DbContext applies automatic query filters:
