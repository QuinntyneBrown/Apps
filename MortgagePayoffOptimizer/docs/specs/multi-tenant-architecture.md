# Multi-Tenant Architecture Specification for MortgagePayoffOptimizer

## Overview

This document describes the multi-tenant architecture implemented in MortgagePayoffOptimizer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

MortgagePayoffOptimizer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`MortgagePayoffOptimizer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`MortgagePayoffOptimizer.Infrastructure/Services/TenantContext.cs`)
...
- **Mortgage**: `TenantId` property for tenant isolation
- **Payment**: `TenantId` property for tenant isolation
- **RefinanceScenario**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `MortgagePayoffOptimizerContext` DbContext applies automatic query filters:
