# Multi-Tenant Architecture Specification for ClassicCarRestorationLog

## Overview

This document describes the multi-tenant architecture implemented in ClassicCarRestorationLog, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

ClassicCarRestorationLog uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`ClassicCarRestorationLog.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`ClassicCarRestorationLog.Infrastructure/Services/TenantContext.cs`)
...
- **Part**: `TenantId` property for tenant isolation
- **WorkLog**: `TenantId` property for tenant isolation
- **PhotoLog**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `ClassicCarRestorationLogContext` DbContext applies automatic query filters:
