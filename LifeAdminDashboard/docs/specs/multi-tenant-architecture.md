# Multi-Tenant Architecture Specification for LifeAdminDashboard

## Overview

This document describes the multi-tenant architecture implemented in LifeAdminDashboard, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

LifeAdminDashboard uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`LifeAdminDashboard.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`LifeAdminDashboard.Infrastructure/Services/TenantContext.cs`)
...
- **AdminTask**: `TenantId` property for tenant isolation
- **Renewal**: `TenantId` property for tenant isolation
- **Deadline**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `LifeAdminDashboardContext` DbContext applies automatic query filters:
