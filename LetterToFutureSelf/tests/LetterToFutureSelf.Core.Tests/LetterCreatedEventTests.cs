// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.

namespace LetterToFutureSelf.Core.Tests;

public class LetterCreatedEventTests
{
    [Test]
    public void Constructor_ValidParameters_CreatesEvent()
    {
        // Arrange
        var letterId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var subject = "Future Reflections";
        var scheduledDate = DateTime.UtcNow.AddYears(1);

        // Act
        var evt = new LetterCreatedEvent
        {
            LetterId = letterId,
            UserId = userId,
            Subject = subject,
            ScheduledDeliveryDate = scheduledDate
        };

        // Assert
        Assert.Multiple(() =>
        {
            Assert.That(evt.LetterId, Is.EqualTo(letterId));
            Assert.That(evt.UserId, Is.EqualTo(userId));
            Assert.That(evt.Subject, Is.EqualTo(subject));
            Assert.That(evt.ScheduledDeliveryDate, Is.EqualTo(scheduledDate));
            Assert.That(evt.Timestamp, Is.Not.EqualTo(default(DateTime)));
        });
    }

    [Test]
    public void Timestamp_IsSetAutomatically()
    {
        // Arrange
        var beforeCreation = DateTime.UtcNow;

        // Act
        var evt = new LetterCreatedEvent
        {
            LetterId = Guid.NewGuid(),
            UserId = Guid.NewGuid()
        };

        var afterCreation = DateTime.UtcNow;

        // Assert
        Assert.That(evt.Timestamp, Is.GreaterThanOrEqualTo(beforeCreation));
        Assert.That(evt.Timestamp, Is.LessThanOrEqualTo(afterCreation));
    }

    [Test]
    public void LetterId_CanBeInitialized_ReturnsCorrectValue()
    {
        // Arrange
        var expectedId = Guid.NewGuid();

        // Act
        var evt = new LetterCreatedEvent { LetterId = expectedId };

        // Assert
        Assert.That(evt.LetterId, Is.EqualTo(expectedId));
    }

    [Test]
    public void UserId_CanBeInitialized_ReturnsCorrectValue()
    {
        // Arrange
        var expectedUserId = Guid.NewGuid();

        // Act
        var evt = new LetterCreatedEvent { UserId = expectedUserId };

        // Assert
        Assert.That(evt.UserId, Is.EqualTo(expectedUserId));
    }

    [Test]
    public void Subject_CanBeInitialized_ReturnsCorrectValue()
    {
        // Arrange
        var expectedSubject = "My Goals for Next Year";

        // Act
        var evt = new LetterCreatedEvent { Subject = expectedSubject };

        // Assert
        Assert.That(evt.Subject, Is.EqualTo(expectedSubject));
    }

    [Test]
    public void ScheduledDeliveryDate_CanBeInitialized_ReturnsCorrectValue()
    {
        // Arrange
        var expectedDate = DateTime.UtcNow.AddMonths(6);

        // Act
        var evt = new LetterCreatedEvent { ScheduledDeliveryDate = expectedDate };

        // Assert
        Assert.That(evt.ScheduledDeliveryDate, Is.EqualTo(expectedDate));
    }
}
