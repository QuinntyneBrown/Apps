# Multi-Tenant Architecture Specification for LetterToFutureSelf

## Overview

This document describes the multi-tenant architecture implemented in LetterToFutureSelf, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

LetterToFutureSelf uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`LetterToFutureSelf.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`LetterToFutureSelf.Infrastructure/Services/TenantContext.cs`)
...
- **Letter**: `TenantId` property for tenant isolation
- **DeliverySchedule**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `LetterToFutureSelfContext` DbContext applies automatic query filters:
