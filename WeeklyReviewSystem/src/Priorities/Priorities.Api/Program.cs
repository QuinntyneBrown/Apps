using Priorities.Core; using Priorities.Infrastructure.Data; using System.Text.Json.Serialization;
var builder = WebApplication.CreateBuilder(args); builder.AddServiceDefaults();
builder.Services.AddControllers().AddJsonOptions(options => { options.JsonSerializerOptions.Converters.Add(new JsonStringEnumConverter()); });
builder.Services.AddEndpointsApiExplorer(); builder.Services.AddSwaggerGen();
builder.AddSqlServerDbContext<PrioritiesDbContext>("WeeklyReviewSystemDb");
builder.Services.AddScoped<IPrioritiesDbContext>(provider => provider.GetRequiredService<PrioritiesDbContext>());
