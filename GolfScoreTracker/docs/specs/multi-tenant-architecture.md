# Multi-Tenant Architecture Specification for GolfScoreTracker

## Overview

This document describes the multi-tenant architecture implemented in GolfScoreTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

GolfScoreTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`GolfScoreTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`GolfScoreTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Round**: `TenantId` property for tenant isolation
- **HoleScore**: `TenantId` property for tenant isolation
- **Handicap**: `TenantId` property for tenant isolation
- **Course**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `GolfScoreTrackerContext` DbContext applies automatic query filters:
