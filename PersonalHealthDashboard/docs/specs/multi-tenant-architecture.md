# Multi-Tenant Architecture Specification for PersonalHealthDashboard

## Overview

This document describes the multi-tenant architecture implemented in PersonalHealthDashboard, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PersonalHealthDashboard uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PersonalHealthDashboard.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PersonalHealthDashboard.Infrastructure/Services/TenantContext.cs`)
...
- **Vital**: `TenantId` property for tenant isolation
- **WearableData**: `TenantId` property for tenant isolation
- **HealthTrend**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PersonalHealthDashboardContext` DbContext applies automatic query filters:
