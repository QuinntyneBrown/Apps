# Multi-Tenant Architecture Specification for CharitableGivingTracker

## Overview

This document describes the multi-tenant architecture implemented in CharitableGivingTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

CharitableGivingTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`CharitableGivingTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`CharitableGivingTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Donation**: `TenantId` property for tenant isolation
- **Organization**: `TenantId` property for tenant isolation
- **TaxReport**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `CharitableGivingTrackerContext` DbContext applies automatic query filters:
