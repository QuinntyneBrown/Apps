# Multi-Tenant Architecture Specification for FamilyPhotoAlbumOrganizer

## Overview

This document describes the multi-tenant architecture implemented in FamilyPhotoAlbumOrganizer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FamilyPhotoAlbumOrganizer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FamilyPhotoAlbumOrganizer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FamilyPhotoAlbumOrganizer.Infrastructure/Services/TenantContext.cs`)
...
- **Photo**: `TenantId` property for tenant isolation
- **Album**: `TenantId` property for tenant isolation
- **Tag**: `TenantId` property for tenant isolation
- **PersonTag**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FamilyPhotoAlbumOrganizerContext` DbContext applies automatic query filters:
