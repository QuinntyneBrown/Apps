# Multi-Tenant Architecture Specification for NutritionLabelScanner

## Overview

This document describes the multi-tenant architecture implemented in NutritionLabelScanner, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

NutritionLabelScanner uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`NutritionLabelScanner.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`NutritionLabelScanner.Infrastructure/Services/TenantContext.cs`)
...
- **Product**: `TenantId` property for tenant isolation
- **NutritionInfo**: `TenantId` property for tenant isolation
- **Comparison**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `NutritionLabelScannerContext` DbContext applies automatic query filters:
