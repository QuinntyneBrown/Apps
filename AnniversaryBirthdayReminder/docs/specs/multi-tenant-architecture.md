# Multi-Tenant Architecture Specification for AnniversaryBirthdayReminder

## Overview

This document describes the multi-tenant architecture implemented in AnniversaryBirthdayReminder, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

AnniversaryBirthdayReminder uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`AnniversaryBirthdayReminder.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`AnniversaryBirthdayReminder.Infrastructure/Services/TenantContext.cs`)
...
- **ImportantDate**: `TenantId` property for tenant isolation
- **Reminder**: `TenantId` property for tenant isolation
- **Gift**: `TenantId` property for tenant isolation
- **Celebration**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `AnniversaryBirthdayReminderContext` DbContext applies automatic query filters:
