# Multi-Tenant Architecture Specification for BookReadingTrackerLibrary

## Overview

This document describes the multi-tenant architecture implemented in BookReadingTrackerLibrary, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

BookReadingTrackerLibrary uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`BookReadingTrackerLibrary.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`BookReadingTrackerLibrary.Infrastructure/Services/TenantContext.cs`)
...
- **Book**: `TenantId` property for tenant isolation
- **ReadingLog**: `TenantId` property for tenant isolation
- **Wishlist**: `TenantId` property for tenant isolation
- **Review**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `BookReadingTrackerLibraryContext` DbContext applies automatic query filters:
