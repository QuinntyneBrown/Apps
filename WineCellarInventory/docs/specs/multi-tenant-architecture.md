# Multi-Tenant Architecture Specification for WineCellarInventory

## Overview

This document describes the multi-tenant architecture implemented in WineCellarInventory, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

WineCellarInventory uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`WineCellarInventory.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`WineCellarInventory.Infrastructure/Services/TenantContext.cs`)
...
- **Wine**: `TenantId` property for tenant isolation
- **TastingNote**: `TenantId` property for tenant isolation
- **DrinkingWindow**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `WineCellarInventoryContext` DbContext applies automatic query filters:
