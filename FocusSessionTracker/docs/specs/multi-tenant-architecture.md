# Multi-Tenant Architecture Specification for FocusSessionTracker

## Overview

This document describes the multi-tenant architecture implemented in FocusSessionTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FocusSessionTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FocusSessionTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FocusSessionTracker.Infrastructure/Services/TenantContext.cs`)
...
- **FocusSession**: `TenantId` property for tenant isolation
- **Distraction**: `TenantId` property for tenant isolation
- **SessionAnalytics**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FocusSessionTrackerContext` DbContext applies automatic query filters:
