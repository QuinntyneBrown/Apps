# Multi-Tenant Architecture Specification for FamilyTreeBuilder

## Overview

This document describes the multi-tenant architecture implemented in FamilyTreeBuilder, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FamilyTreeBuilder uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FamilyTreeBuilder.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FamilyTreeBuilder.Infrastructure/Services/TenantContext.cs`)
...
- **Person**: `TenantId` property for tenant isolation
- **Relationship**: `TenantId` property for tenant isolation
- **Story**: `TenantId` property for tenant isolation
- **FamilyPhoto**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FamilyTreeBuilderContext` DbContext applies automatic query filters:
