# Multi-Tenant Architecture Specification for InjuryPreventionRecoveryTracker

## Overview

This document describes the multi-tenant architecture implemented in InjuryPreventionRecoveryTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

InjuryPreventionRecoveryTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`InjuryPreventionRecoveryTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`InjuryPreventionRecoveryTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Injury**: `TenantId` property for tenant isolation
- **RecoveryExercise**: `TenantId` property for tenant isolation
- **Milestone**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `InjuryPreventionRecoveryTrackerContext` DbContext applies automatic query filters:
