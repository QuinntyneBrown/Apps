# Multi-Tenant Architecture Specification for CryptoPortfolioManager

## Overview

This document describes the multi-tenant architecture implemented in CryptoPortfolioManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

CryptoPortfolioManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`CryptoPortfolioManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`CryptoPortfolioManager.Infrastructure/Services/TenantContext.cs`)
...
- **Wallet**: `TenantId` property for tenant isolation
- **CryptoHolding**: `TenantId` property for tenant isolation
- **Transaction**: `TenantId` property for tenant isolation
- **TaxLot**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `CryptoPortfolioManagerContext` DbContext applies automatic query filters:
