...
public class AchievementsController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<AchievementDto>>> GetAchievements(
        ...
        [FromQuery] AchievementType? achievementType,
    ...
    [HttpPut("{achievementId:guid}/featured")]
    [ProducesResponseType(typeof(AchievementDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<AchievementDto>> ToggleFeatured(Guid achievementId)
    {
        _logger.LogInformation("Toggling featured for achievement {AchievementId}", achievementId);

        var result = await _mediator.Send(new ToggleFeaturedCommand { AchievementId = achievementId });
