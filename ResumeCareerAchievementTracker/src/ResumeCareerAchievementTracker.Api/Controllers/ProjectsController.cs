...
public class ProjectsController : ControllerBase
{
    ...
    [HttpPut("{projectId:guid}/featured")]
    [ProducesResponseType(typeof(ProjectDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<ProjectDto>> ToggleFeatured(Guid projectId)
    {
        _logger.LogInformation("Toggling featured for project {ProjectId}", projectId);

        var result = await _mediator.Send(new ToggleProjectFeaturedCommand { ProjectId = projectId });
