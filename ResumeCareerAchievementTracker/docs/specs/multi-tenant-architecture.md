# Multi-Tenant Architecture Specification for ResumeCareerAchievementTracker

## Overview

This document describes the multi-tenant architecture implemented in ResumeCareerAchievementTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

ResumeCareerAchievementTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`ResumeCareerAchievementTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`ResumeCareerAchievementTracker.Infrastructure/Services/TenantContext.cs`)
...
The `ResumeCareerAchievementTrackerContext` DbContext applies automatic query filters:
