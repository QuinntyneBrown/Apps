# Multi-Tenant Architecture Specification for FreelanceProjectManager

## Overview

This document describes the multi-tenant architecture implemented in FreelanceProjectManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FreelanceProjectManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FreelanceProjectManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FreelanceProjectManager.Infrastructure/Services/TenantContext.cs`)
...
- **Client**: `TenantId` property for tenant isolation
- **TimeEntry**: `TenantId` property for tenant isolation
- **Invoice**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FreelanceProjectManagerContext` DbContext applies automatic query filters:
