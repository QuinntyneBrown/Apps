# Multi-Tenant Architecture Specification for PokerGameTracker

## Overview

This document describes the multi-tenant architecture implemented in PokerGameTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PokerGameTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PokerGameTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PokerGameTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Hand**: `TenantId` property for tenant isolation
- **Bankroll**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PokerGameTrackerContext` DbContext applies automatic query filters:
