# Multi-Tenant Architecture Specification for RetirementSavingsCalculator

## Overview

This document describes the multi-tenant architecture implemented in RetirementSavingsCalculator, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

RetirementSavingsCalculator uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`RetirementSavingsCalculator.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`RetirementSavingsCalculator.Infrastructure/Services/TenantContext.cs`)
...
- **RetirementScenario**: `TenantId` property for tenant isolation
- **Contribution**: `TenantId` property for tenant isolation
- **WithdrawalStrategy**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `RetirementSavingsCalculatorContext` DbContext applies automatic query filters:
