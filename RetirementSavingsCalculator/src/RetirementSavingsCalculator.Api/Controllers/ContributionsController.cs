...
public class ContributionsController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<ContributionDto>>> GetContributions(
        ...
        [FromQuery] bool? isEmployerMatch,
        [FromQuery] decimal? minAmount,
        [FromQuery] decimal? maxAmount)
    {
        _logger.LogInformation("Getting contributions for scenario {RetirementScenarioId}", retirementScenarioId);

        var result = await _mediator.Send(new GetContributionsQuery
        {
            RetirementScenarioId = retirementScenarioId,
            StartDate = startDate,
            EndDate = endDate,
            AccountName = accountName,
            IsEmployerMatch = isEmployerMatch,
            MinAmount = minAmount,
            MaxAmount = maxAmount,
        });
    ...
    {
        _logger.LogInformation("Creating contribution of {Amount} for scenario {RetirementScenarioId}", command.Amount, command.RetirementScenarioId);
    ...
    {
        if (contributionId != command.ContributionId)
        {
