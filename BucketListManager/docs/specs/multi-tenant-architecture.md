# Multi-Tenant Architecture Specification for BucketListManager

## Overview

This document describes the multi-tenant architecture implemented in BucketListManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

BucketListManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`BucketListManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`BucketListManager.Infrastructure/Services/TenantContext.cs`)
...
- **BucketListItem**: `TenantId` property for tenant isolation
- **Milestone**: `TenantId` property for tenant isolation
- **Memory**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `BucketListManagerContext` DbContext applies automatic query filters:
