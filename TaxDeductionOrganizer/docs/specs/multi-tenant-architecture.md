# Multi-Tenant Architecture Specification for TaxDeductionOrganizer

## Overview

This document describes the multi-tenant architecture implemented in TaxDeductionOrganizer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

TaxDeductionOrganizer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`TaxDeductionOrganizer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`TaxDeductionOrganizer.Infrastructure/Services/TenantContext.cs`)
...
- **Deduction**: `TenantId` property for tenant isolation
- **Receipt**: `TenantId` property for tenant isolation
- **TaxYear**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `TaxDeductionOrganizerContext` DbContext applies automatic query filters:
