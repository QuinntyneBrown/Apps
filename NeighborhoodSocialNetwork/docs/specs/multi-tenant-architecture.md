# Multi-Tenant Architecture Specification for NeighborhoodSocialNetwork

## Overview

This document describes the multi-tenant architecture implemented in NeighborhoodSocialNetwork, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

NeighborhoodSocialNetwork uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`NeighborhoodSocialNetwork.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`NeighborhoodSocialNetwork.Infrastructure/Services/TenantContext.cs`)
...
- **Neighbor**: `TenantId` property for tenant isolation
- **Event**: `TenantId` property for tenant isolation
- **Recommendation**: `TenantId` property for tenant isolation
- **Message**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `NeighborhoodSocialNetworkContext` DbContext applies automatic query filters:
