# Multi-Tenant Architecture Specification for GroceryShoppingListApp

## Overview

This document describes the multi-tenant architecture implemented in GroceryShoppingListApp, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

GroceryShoppingListApp uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`GroceryShoppingListApp.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`GroceryShoppingListApp.Infrastructure/Services/TenantContext.cs`)
...
- **GroceryList**: `TenantId` property for tenant isolation
- **GroceryItem**: `TenantId` property for tenant isolation
- **Store**: `TenantId` property for tenant isolation
- **PriceHistory**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `GroceryShoppingListAppContext` DbContext applies automatic query filters:
