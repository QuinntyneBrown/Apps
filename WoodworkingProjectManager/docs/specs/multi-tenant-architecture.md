# Multi-Tenant Architecture Specification for WoodworkingProjectManager

## Overview

This document describes the multi-tenant architecture implemented in WoodworkingProjectManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

WoodworkingProjectManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`WoodworkingProjectManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`WoodworkingProjectManager.Infrastructure/Services/TenantContext.cs`)
...
- **Tool**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `WoodworkingProjectManagerContext` DbContext applies automatic query filters:
