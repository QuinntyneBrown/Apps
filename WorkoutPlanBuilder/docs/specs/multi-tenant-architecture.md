# Multi-Tenant Architecture Specification for WorkoutPlanBuilder

## Overview

This document describes the multi-tenant architecture implemented in WorkoutPlanBuilder, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

WorkoutPlanBuilder uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`WorkoutPlanBuilder.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`WorkoutPlanBuilder.Infrastructure/Services/TenantContext.cs`)
...
- **Workout**: `TenantId` property for tenant isolation
- **Exercise**: `TenantId` property for tenant isolation
- **ProgressRecord**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `WorkoutPlanBuilderContext` DbContext applies automatic query filters:
