# Multi-Tenant Architecture Specification for SkillDevelopmentTracker

## Overview

This document describes the multi-tenant architecture implemented in SkillDevelopmentTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

SkillDevelopmentTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`SkillDevelopmentTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`SkillDevelopmentTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Skill**: `TenantId` property for tenant isolation
- **Course**: `TenantId` property for tenant isolation
- **Certification**: `TenantId` property for tenant isolation
- **LearningPath**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `SkillDevelopmentTrackerContext` DbContext applies automatic query filters:
