# Multi-Tenant Architecture Specification for HouseholdBudgetManager

## Overview

This document describes the multi-tenant architecture implemented in HouseholdBudgetManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

HouseholdBudgetManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`HouseholdBudgetManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`HouseholdBudgetManager.Infrastructure/Services/TenantContext.cs`)
...
The `HouseholdBudgetManagerContext` DbContext applies automatic query filters:
