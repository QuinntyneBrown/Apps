# Multi-Tenant Architecture Specification for HomeGymEquipmentManager

## Overview

This document describes the multi-tenant architecture implemented in HomeGymEquipmentManager, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

HomeGymEquipmentManager uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`HomeGymEquipmentManager.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`HomeGymEquipmentManager.Infrastructure/Services/TenantContext.cs`)
...
- **Equipment**: `TenantId` property for tenant isolation
- **Maintenance**: `TenantId` property for tenant isolation
- **WorkoutMapping**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `HomeGymEquipmentManagerContext` DbContext applies automatic query filters:
