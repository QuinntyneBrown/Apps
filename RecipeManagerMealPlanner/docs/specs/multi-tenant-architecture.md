# Multi-Tenant Architecture Specification for RecipeManagerMealPlanner

## Overview

This document describes the multi-tenant architecture implemented in RecipeManagerMealPlanner, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

RecipeManagerMealPlanner uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`RecipeManagerMealPlanner.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`RecipeManagerMealPlanner.Infrastructure/Services/TenantContext.cs`)
...
- **Recipe**: `TenantId` property for tenant isolation
- **MealPlan**: `TenantId` property for tenant isolation
- **Ingredient**: `TenantId` property for tenant isolation
- **ShoppingList**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `RecipeManagerMealPlannerContext` DbContext applies automatic query filters:
