# Multi-Tenant Architecture Specification for DocumentVaultOrganizer

## Overview

This document describes the multi-tenant architecture implemented in DocumentVaultOrganizer, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

DocumentVaultOrganizer uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`DocumentVaultOrganizer.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`DocumentVaultOrganizer.Infrastructure/Services/TenantContext.cs`)
...
- **Document**: `TenantId` property for tenant isolation
- **DocumentCategory**: `TenantId` property for tenant isolation
- **ExpirationAlert**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `DocumentVaultOrganizerContext` DbContext applies automatic query filters:
