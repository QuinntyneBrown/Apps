# Multi-Tenant Architecture Specification for GiftIdeaTracker

## Overview

This document describes the multi-tenant architecture implemented in GiftIdeaTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

GiftIdeaTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`GiftIdeaTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`GiftIdeaTracker.Infrastructure/Services/TenantContext.cs`)
...
- **GiftIdea**: `TenantId` property for tenant isolation
- **Recipient**: `TenantId` property for tenant isolation
- **Purchase**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `GiftIdeaTrackerContext` DbContext applies automatic query filters:
