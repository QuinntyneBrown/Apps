# Multi-Tenant Architecture Specification for PhotographySessionLogger

## Overview

This document describes the multi-tenant architecture implemented in PhotographySessionLogger, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PhotographySessionLogger uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PhotographySessionLogger.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PhotographySessionLogger.Infrastructure/Services/TenantContext.cs`)
...
- **Session**: `TenantId` property for tenant isolation
- **Photo**: `TenantId` property for tenant isolation
- **Gear**: `TenantId` property for tenant isolation
- **Project**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PhotographySessionLoggerContext` DbContext applies automatic query filters:
