# Multi-Tenant Architecture Specification for PersonalLoanComparisonTool

## Overview

This document describes the multi-tenant architecture implemented in PersonalLoanComparisonTool, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PersonalLoanComparisonTool uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PersonalLoanComparisonTool.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PersonalLoanComparisonTool.Infrastructure/Services/TenantContext.cs`)
...
- **Loan**: `TenantId` property for tenant isolation
- **Offer**: `TenantId` property for tenant isolation
- **PaymentSchedule**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PersonalLoanComparisonToolContext` DbContext applies automatic query filters:
