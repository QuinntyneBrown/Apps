# Multi-Tenant Architecture Specification for FamilyVacationPlanner

## Overview

This document describes the multi-tenant architecture implemented in FamilyVacationPlanner, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FamilyVacationPlanner uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FamilyVacationPlanner.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FamilyVacationPlanner.Infrastructure/Services/TenantContext.cs`)
...
- **Trip**: `TenantId` property for tenant isolation
- **Itinerary**: `TenantId` property for tenant isolation
- **Booking**: `TenantId` property for tenant isolation
- **VacationBudget**: `TenantId` property for tenant isolation
- **PackingList**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FamilyVacationPlannerContext` DbContext applies automatic query filters:
