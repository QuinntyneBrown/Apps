# Multi-Tenant Architecture Specification for SalaryCompensationTracker

## Overview

This document describes the multi-tenant architecture implemented in SalaryCompensationTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

SalaryCompensationTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`SalaryCompensationTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`SalaryCompensationTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Compensation**: `TenantId` property for tenant isolation
- **Benefit**: `TenantId` property for tenant isolation
- **MarketComparison**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `SalaryCompensationTrackerContext` DbContext applies automatic query filters:
