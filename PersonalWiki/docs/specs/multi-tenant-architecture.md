# Multi-Tenant Architecture Specification for PersonalWiki

## Overview

This document describes the multi-tenant architecture implemented in PersonalWiki, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

PersonalWiki uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`PersonalWiki.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`PersonalWiki.Infrastructure/Services/TenantContext.cs`)
...
- **WikiPage**: `TenantId` property for tenant isolation
- **PageRevision**: `TenantId` property for tenant isolation
- **WikiCategory**: `TenantId` property for tenant isolation
- **PageLink**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `PersonalWikiContext` DbContext applies automatic query filters:
