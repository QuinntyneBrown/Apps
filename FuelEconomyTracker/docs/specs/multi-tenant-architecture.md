# Multi-Tenant Architecture Specification for FuelEconomyTracker

## Overview

This document describes the multi-tenant architecture implemented in FuelEconomyTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

FuelEconomyTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`FuelEconomyTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`FuelEconomyTracker.Infrastructure/Services/TenantContext.cs`)
...
- **FillUp**: `TenantId` property for tenant isolation
- **Trip**: `TenantId` property for tenant isolation
- **Vehicle**: `TenantId` property for tenant isolation
- **EfficiencyReport**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `FuelEconomyTrackerContext` DbContext applies automatic query filters:
