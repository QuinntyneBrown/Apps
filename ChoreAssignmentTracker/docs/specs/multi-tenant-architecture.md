# Multi-Tenant Architecture Specification for ChoreAssignmentTracker

## Overview

This document describes the multi-tenant architecture implemented in ChoreAssignmentTracker, enabling complete data isolation between different tenants (organizations) sharing the same application instance.

## Architecture

### Tenant Isolation Strategy

ChoreAssignmentTracker uses a **shared database with tenant ID column** approach, where:
...
#### 1. ITenantContext Interface (`ChoreAssignmentTracker.Core/ITenantContext.cs`)
...
#### 2. TenantContext Service (`ChoreAssignmentTracker.Infrastructure/Services/TenantContext.cs`)
...
- **Chore**: `TenantId` property for tenant isolation
- **Assignment**: `TenantId` property for tenant isolation
- **Reward**: `TenantId` property for tenant isolation
- **FamilyMember**: `TenantId` property for tenant isolation

#### 4. Global Query Filters

The `ChoreAssignmentTrackerContext` DbContext applies automatic query filters:
